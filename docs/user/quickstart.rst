...
``openwisp-monitoring`` APK files and ``sha256.manifest`` from `the latest
build
<https://downloads.openwisp.io/?prefix=openwisp-monitoring/latest/>`_.
Verify the APK checksums before installing them. ``apk`` verifies their
signatures with the installed public key.
